...
directory names — remain songmaker's decision, installed through
...
- **Tool-surface verification, one gate per call shape, not per caller**: `_build_cli_cmd()` is reached through `_call_cli()`/`_acall_cli()`, and those two call `verify_no_builtin_cli_tools()`/`averify_no_builtin_cli_tools()` themselves before building anything — so `chat_api.py`'s legacy endpoint and the lyrical-coherence judge are covered by the same gate without either having called it themselves (#351). Since #837 it has one further reach: `_cowriter_cli_command()` builds it for a co-writer turn when the host declared no MCP server (`ProviderRuntimeConfig.mcp_server is None`). That route is gated too, and by the matching expectation — `verify_cli_tool_surface()` delegates to `averify_no_builtin_cli_tools()` for exactly that case, so the shape that is verified is always the shape that runs. `_build_mcp_cli_cmd()` is reached only through the two MCP-attached entry points (`acall_claude_with_mcp`/`acall_claude_with_mcp_stream`), which call `verify_cli_tool_surface()` themselves the same way — a separate gate, not routed through `_call_cli()`/`_acall_cli()`. On a cache **miss**, the CLI is a bind-mounted binary that updates itself, so the relevant gate starts a session with exactly the flags above and reads the tool names — and the `slash_commands` list — out of the CLI's own `system` init event (`subtype` checked too, not just `type`); a cache **hit** returns the remembered verdict without starting a session at all. The co-writer's probe attaches the real `--mcp-config` and requires the reported tool set to equal, exactly, the `mcp__songmaker__*` names — a hand-maintained literal in `songmaker_cli/cowriter/mcp_spec.py`, not imported from `mcp_server/server.py` (that would pull the `mcp` package into containers that do not carry it), kept honest by a test that compares the two — not merely a subset check, so a tool going missing is caught the same way an extra one is, but an unexpected tool or a reachable slash command is *always* a permanent mismatch regardless of whether the MCP connection itself came up (a CLI reporting `tools=["Bash"]` with its MCP connection also down is confirmed dangerous, not merely unverifiable). The tool-free probe requires no tool and no slash command at all, and deliberately never attaches `--mcp-config`: registering and listing MCP tools touches no database (only a tool *call* does), so that is not why this probe stays separate — it needs the `mcp` extra, which the scoring-worker container does not install; that container does have a reachable database (it writes scores back to it), so the database is not its actual gap (confirmed live: a missing `mcp` package makes the MCP connection fail, and a failed connection reports zero tools, the same shape a clean tool-free CLI reports).
  ...
  Tool-surface probes use the single bounded process layer, `agent_providers.process.run_cli_bounded`, for spawn, bounded stdin/stdout exchange, output limits, and process-group cleanup. The sync gate calls it directly; the async gate reaches that same gate via `asyncio.to_thread`, keeping a stuck spawn off the event loop without a third probe-specific thread. `agent_providers/claude/provider.py` keeps the policy around that runner: per-build verdict/failure cache, ordinary and zombie TTLs, single-flight, and the shared Claude process pool. It reserves a slot before invoking the runner, binds it in `on_spawned`, and releases it only in `on_reaped`; a `DEADLINE_BEFORE_SPAWN` may return while a later spawn is still live, so its result must never release that reservation. A `became_zombie` outcome wins before parsing the init event, receives the zombie TTL, and blocks a new process when the shared pool is full. The gate resolves the CLI's symlink once, at probe time, and hands that literal path back for the real turn to run — so a self-update landing between the probe and the turn cannot execute a build that was never checked. All four gated command paths — `acall_claude_with_mcp`, `acall_claude_with_mcp_stream`, `_call_cli` and `_acall_cli` — carry that assurance as a test that compares the probed command line against the executed one, so a future caller that resolves the binary a second time fails a test rather than shipping.
...
with Code Mode enabled only for this image run. Its one variable part is the
admin-stored cover model, appended as `--model <slug>` when that setting is
not empty. Each run creates a private
...
Two packages spawn *external* child processes that must not inherit every secret in the parent's environment: `agent_providers` (the Claude, Grok and Codex CLIs) and `acestep_worker.subprocess_runner` (the ACE-Step HTTP subprocess). The provider layer has one scrubber, `agent_providers.process.scrubbed_env()`, and all three CLI spawners go through it — the Claude backends in `agent_providers/claude/provider.py`, `agent_providers/grok/transport.py`, `agent_providers/codex/transport.py`, and the bounded runner's own `_child_env()`, which adds a spawner's explicit variables on top of that scrubbed base rather than beside it. Both packages scrub `os.environ.copy()` with a `SECRET_ENV_KEYS` tuple before passing `env=` to the child, covering `ANTHROPIC_API_KEY`, `XAI_API_KEY`, `OPENAI_API_KEY`, `SESSION_SECRET`, `SONGMAKER_INTERNAL_TOKEN`, `DATABASE_URL`, `REDIS_URL`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `HF_TOKEN`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `GRAFANA_USER`, and `GRAFANA_PASSWORD`. A login is scrubbed as a pair — the name half is no secret on its own, but handing a child process one half of a credential buys nothing.
...
`acestep_worker` cannot import from `songmaker_cli` (engine packages are independent — see CLAUDE.md), so each package keeps its own `SECRET_ENV_KEYS` tuple: `songmaker_cli/constants.py` and `acestep_worker/constants.py`. `agent_providers` may import neither, so the provider layer scrubs with the list its host injects on `ProviderRuntimeConfig.secret_env_keys` — songmaker fills it from its own constant in `agent_runtime.py` (#836), which makes the injected runtime a third owner of the same list. `tests/test_secret_scrub_parity.py` pins all three against the same literal expected set of keys — so they cannot silently drift apart the way the first two did before issue #157 (the Claude CLI child inherited `SONGMAKER_INTERNAL_TOKEN` because the two lists disagreed), no list can quietly shrink to empty and still pass, and a drifted list names its own owner.
