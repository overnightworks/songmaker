...
GenerateRunner = Any
TrainLoraRunner = Any
GPU_HOLD_REJECTED_DETAIL = "GPU hold token is invalid"
...
class _WorkerRoutes:
    ...
    async def renew_hold(self, req: GpuHoldTokenRequest) -> None:
        if not await renew_gpu_hold(
            ...
            raise HTTPException(status_code=409, detail=GPU_HOLD_REJECTED_DETAIL)
...
def _register_worker_routes(router: APIRouter, routes: _WorkerRoutes) -> None:
    ...
    router.add_api_route(
        ...
        status_code=204,
        dependencies=authenticated,
        responses={409: {"description": GPU_HOLD_REJECTED_DETAIL}},
    )
    router.add_api_route(
        "/gpu_hold/release",
        routes.release_hold,
